...
class Controller:
    ...
    def __init__(self):
        ...
        if getattr(sys, 'frozen', False):
            # 當程式被打包為 exe 時，使用此路徑
            self.current_dir = os.path.dirname(sys.executable)  # 獲取可執行文件所在的目錄
        else:
        ...
        if not self.window_id:
            ...
            quit()
        self.process_handle, temp_link = self.get_process_handle_from_hwnd(self.window_id) # 打開進程 進程,路徑
        suffixs_lates = self.json讀取(self.current_dir+r'\詞墬.json', 'r')
        self.suffixs_lates = json.loads(suffixs_lates) # 詞墜中文列表
        self.found_names, self.found_scripts = self.find_cards_with_series(temp_link+r'\system\itemInfo_true.lub') # 讀取文本並存入 名稱 說明
        pass
    ...
    def show_selection_box(self, event):
        # 顯示選擇框並填充選項
        options = self.suffixs_lates
        self.ui.tk_selection_box.delete(0, END)
        for option in options:
            data = self.suffixs_lates[option]
            processed_string = data.split("。可強化次數：")[0]
            # 將 %d 替換為 x，並將 %% 替換為 %
            processed_string = processed_string.replace("%d", "x").replace("%%", "%")
            self.ui.tk_selection_box.insert(END, f'{option}: {processed_string}')
        # 設置位置
    ...
    def on_select(self, event):
        ...
        if not self.ui.tk_selection_box.winfo_viewable():
            ...
            if number_part:
                self.ui.tk_input_m2w3agz9.insert(END, number_part[0] + ",")  # 插入數值部分到輸入框
            self.ui.tk_selection_box.place_forget()  # 隱藏選擇框
    ...
    def check_machine_code(self):
        ...
        # 這裡假設你有一個預定義的機器碼進行比較
        predefined_machine_code = [ 'b3adfd3e84f1d6cc','8d11da0eeba50eb8','d73df39c1d230cd8','e4295f9ae9ab14a7','06b9cd415d477f16',
    ...
    def read_memory(self, address, size=4, data_type='int'):
        ...
        if data_type == 'int':
        ...
        elif data_type == 'short':  # 有符號 2 字節
            addr = ctypes.c_int16()
            size = 2  # 2 字節大小
        elif data_type == 'ushort':  # 無符號 2 字節
            ...
            size = 8  # double 類型需要 8 字節
        elif data_type == 'Big5':  # 處理 Big5 編碼的字符串
            buffer = ctypes.create_string_buffer(size)
            addr = ctypes.c_size_t()
        else:
            addr = ctypes.c_ulong()  # 默認使用 4 字節無符號 long
        # 如果是 Big5 編碼的字符串類型
        if data_type == 'Big5':
            ReadProcessMemory(self.process_handle.handle, ctypes.c_void_p(mem_address), buffer, size, ctypes.byref(addr))
            return buffer.raw  # 返回讀取的字節數據
        # 其他類型讀取內存
        success = ReadProcessMemory(int(self.process_handle), mem_address, ctypes.byref(addr), size, None)
        if not success:
            raise ctypes.WinError()  # 如果讀取失敗，拋出異常
        return addr.value  # 返回讀取到的值
    ...
    def read_string(self, address, offsets=[], max_length=100, encoding='Big5'):
        ...
        if offsets:
            for offset in offsets:
                ...
                address = pointer_value + offset  # 指針+偏移量
    ...
    def generate_string(self,template_id):
        ...
        """
        if template_id != 0:
            if '%d' in self.suffixs_lates[str(template_id)]:
                if str(template_id) in self.suffixs_lates:
                    return self.suffixs_lates[str(template_id)]
            return self.suffixs_lates[str(template_id)]
        return ''
    ...
    def is_fullwidth(self,char):
        ...
        return (
            ...
            '\u3040' <= char <= '\u309F' or  # 平假名
            '\u30A0' <= char <= '\u30FF' or  # 
            '\u2161' <= char <= '\u2162'      # 羅馬數字 I、II、III
        )

    def check_string(self,string, length=0):
        """
        補足字串長度 字串,長度
        """
        if length == 0:
            return string
        ss = sum(1 if self.is_fullwidth(c) else 0.5 for c in string)
        while length > ss:
    ...
    def search_data(self):
        if win32gui.IsWindow(self.window_id):
            ...
            if 倉庫 < 1 :
                ...
                for i in range(1,倉庫+1):
                    ...
                    item = int(self.read_string(hex(exe_addint) )) # 物品ID
                    if item : # 有物品ID 且 搜尋ID也有
                        ...
                        if search_type == '個人':
                            ...
                            個人 = None
                        if (types == 4 or types == 5) and not 個人:
                            ii += 1
    ...
    def adjust_column_width(self, treeview, columns, font_obj):
        """根據內容自動調整列寬"""
        for col in columns:
            max_width = columns[col]  # 初始寬度為預設值
            # 遍歷此列的所有項目，找到寬度最大值
            for item in treeview.get_children():
                text = treeview.set(item, col)
                max_width = max(max_width, font_obj.measure(text) + 20)  # 增加些許空隙
    ...
    def research(self,evt=''):
        ...
        if checked:  # 检查复选框是否被选中
            self.search_data()
            self.ui.after(5000, self.research)  # 每5秒再次调用 start_timer
    def outputJson(self, evt):
        ...
        with open(json_file_path, 'r', encoding='utf-8') as f:
            data = json.load(f)
        # 确保 self.itemss 是一个字典
        if self.itemss:
            # 更新 "倉庫" 的内容
            selected_type = self.ui.tk_select_box_m2w2v0f5.get()
            if selected_type == "倉庫":
                data[self.倉庫名稱] = self.itemss  # 直接赋值给字典
            elif selected_type == "個人":
                data["個人"] = self.itemss  # 直接赋值给字典
            # 将更新后的数据写回 JSON 文件
            with open(json_file_path, 'w', encoding='utf-8') as f:
    ...
    def refresh(self,evt=""):
        ...
        if self.itemss :
            ...
            for val in self.itemss:
                # print(f'\n{valuess}')
                suffix_匹配 = any(suffix in [val['suffix_ID1'], val['suffix_ID2'], val['suffix_ID3'], val['suffix_ID4'], val['suffix_ID5']] for suffix in list1)
                ...
                # print(f'suffix_匹配 {suffix_匹配} refine_匹配{refine_匹配} suffix_veal2_匹配 {suffix2_匹配}')
    ...
    def hwnd_binding(self,evt):
        ...
        if not self.window_id:
            ...
            self.ui.title(f'裝備管理工具 目前尋找 {char_name} 的倉庫')
    ...
    def on_double_click(self, event):
        ...
        if selected_item:
            ...
            self.ui.clipboard_append(filtered_values)  # 追加內容到剪貼板