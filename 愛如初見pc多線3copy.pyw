...
def reg_dm(註冊碼,附加碼):
    ...
    if dm_ret == -1 :
        msg = '可能防火牆攔截,如果可以正常訪問大漠插件網站，那就可以肯定是被防火牆攔截'
    elif dm_ret == -2 :
        msg = '進程沒有以管理員方式運行'
    elif dm_ret == 0 :
        msg = '未知錯誤'
        ...
        msg = '綁定了本機器，但是賬戶餘額不足50元'
    elif dm_ret == 4 :
        msg = '註冊碼錯誤'
    elif dm_ret == 5 :
        msg = '你的機器或者IP在黑名單列表中或者不在白名單列表中'
    elif dm_ret == 6 :
        msg = '非法使用插件. 一般出現在定制插件時，使用了和綁定的用戶名不同的註冊碼'
    elif dm_ret == 7 :
        msg = '你的帳號因為非法使用被封禁'
    elif dm_ret == 8 :
        msg = 'ver_info不在你設置的附加白名單中'
    elif dm_ret == 77 :
        msg = '機器碼或者IP因為非法使用，而被封禁'
    elif dm_ret == 777 :
        msg = '同一個機器碼註冊次數超過了服務器限制'
    elif dm_ret == -8 :
        msg = '版本附加信息長度超過了20'
    elif dm_ret == -9 :
        msg = '版本附加信息裡包含了非法字母'
        ...
        msg = '\t成功註冊,使用腳本插件需要付費,依照使用時間計價,請勿給他人使用 ^_^y'
