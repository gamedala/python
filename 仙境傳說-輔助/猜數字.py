    ...
    while True: # 迴圈
        if job.記憶體數值("01173108") > 0: # 對話
            ...
            if job.記憶體數值("01173230") > 0: # 輸入框
                ...
                # print(f'輸入數值: {guess}')
                # msgxy = ["",job.記憶體數值("[<香水仙境_2024-08-18.exe>+00D73108]+1c")+17,job.記憶體數值("[<香水仙境_2024-08-18.exe>+00D73108]+20")+14]
                # print(msgxy)
                while A is None and job.記憶體數值("01173108") > 0: # 迴圈尋找 A B
                    job.按鍵(13)
                    sleep(0.25)
                    if job.記憶體數值("01173230") > 0: # 出現輸入框 放棄本輪
