...
class Operation:
    ...
    def 記憶體字串(self,位置,str):
        return self.dm.ReadString(self.hwnd, 位置, 0, str)
    ...
    while True: # 迴圈
        if job.記憶體數值("01173108") > 0: # 對話
            if job.記憶體數值("01173230") > 0: # 輸入框
                romsg = job.記憶體字串("[[[[<香水仙境_2024-08-18.exe>+00D73108]+94]+c0]+60]+0", 99)
                guess = re.search(r"\^FF0000 (\d+) \^000000", romsg).group(1)
                長度 = len(f"{guess}")
                print(f'\r驗證碼: {guess} 長度: {長度} ',end="")
                job.寫記憶體字串("013EF0CC", f'{guess}') # 字符串
                job.寫記憶體數值("013EF0DC", 長度) # 長度
            job.按鍵(13)
            sleep(0.05)
        else: # 無對話
            job.找字([91,66,893,596],"幸運輪盤","000000-000000",[35,40],True)
            sleep(0.2)
        sleep(0.05)